...
import { GET_POST_DATA, UPDATE_PRE_RESULT } from "apis/queries";
...
function PreResult({ Id, partCode }) {
  ...
  useEffect(() => {
    if (preTableData.data && prePostStore.noOFSamples.length !== 0 && !newValues) {
      if (preTableData.data.crvtPostTestTableByComponentId?.ptParameter) {
        setPdata(true);
        let tempArray = {
          ...JSON.parse(preTableData.data.crvtPostTestTableByComponentId?.ptParameter),
        };
                                      ...
                                      >
                                        {show ? (
                                          <TextField
                                            ...
                                            // value={values}
                                            onChange={(e) => handleValues(e, pindex, cindex, c_valuesIndex)
                                            }
                                          />
                                        ) : (
                                          <MDTypography
